...
exports.Schiebegardine_different = class Schiebegardine_different {
    ...
    async configureSchiebegardineDifferent(testcase, paneele) {
        ...
        let bediengriffe_mwst_total = (jsonLogic.apply({ '*': [bediengriffe_mwst, testcase.anzahl] })).toFixed(2)
        ...
        if (testcase.pan_anpassen) {
            ...
            var stoffe = [testcase.stoff_1, testcase.stoff_2, testcase.stoff_3, testcase.stoff_4, testcase.stoff_5, testcase.stoff_6, testcase.stoff_7, testcase.stoff_8]
            ...
            for (var j = 1, i = 0; i <= 7; i++, j++) {
                ...
                if (typeof stoffe[i] !== "undefined") {
                    await this.page.locator('.optional_paneele_wrapper > :nth-child(' + j + ') input[type="button"]').click();
                    await this.page.locator('#material-collection > ul > li').getByText(stoffe[i]).first().click();
                    await this.page.getByText(/Auswahl übernehmen/).first().click();
                }
