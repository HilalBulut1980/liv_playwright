...
exports.Holzjalousie = class Holzjalousie {

    constructor(page) {
        this.page = page;
    }

    async configureHolzjalousie(testcase) {
        ...
        let leiterband_red = (jsonLogic.apply({ '*': [testcase.discount, testcase.leiterband] })).toFixed(2);
        let leiterband_red_mwst = (jsonLogic.apply({ '*': [jsonLogic.apply({ '/': [leiterband_red, 119] }), testcase.vat] })).toFixed(2);
        let leiterband_mwst = (jsonLogic.apply({ '*': [jsonLogic.apply({ '/': [testcase.leiterband, 119] }), testcase.vat] })).toFixed(2);

        //Konfigurator & Warenkorb
        let streichPreis = (jsonLogic.apply({ '+': [testcase.grundpreis, testcase.kette, testcase.pendel, testcase.klemm_oben, testcase.klemm_unten, testcase.leiterband] })).toFixed(2)
        let streichPreisTotal = (jsonLogic.apply({ '*': [streichPreis, testcase.anzahl] })).toFixed(2)
        let redPreis = (jsonLogic.apply({ '+': [grundPreis_red, testcase.kette, testcase.pendel, testcase.klemm_oben, testcase.klemm_unten, leiterband_red] })).toFixed(2)
        ...
        // check Startpreise on /holzjalousien  
        ...
        if (testcase.produktgruppe == "70mm") {
        ...
        if (typeof testcase.bedientyp != "undefined") {
            await this.page.locator('.bedienung > div > ul').locator(":scope > *")
                .getByText(testcase.bedientyp).first().click();
        }

        // set Farbe von Kette und Blende
        if (typeof testcase.farbe_ketteUndBlende != "undefined") {
            await this.page.locator('.farbe > :nth-child(3) > ul').locator(":scope > *")
                .getByText(testcase.farbe_ketteUndBlende).first().click();
        }

        // set Farbe Kette
        if (typeof testcase.farbe_kette != "undefined") {
        ...
        if (typeof testcase.farbe_seitenblende != "undefined") {
            await this.page.locator('.farbe > :nth-child(7) > ul').locator(":scope > *")
        ...
        // set Lemellenverbindung
        if (typeof testcase.lamellenverbindung != "undefined") {
            await this.page.locator('.lamellenverbindung > div > ul').locator(":scope > *")
                .getByText(testcase.lamellenverbindung).first().click();
        }

        // set Farbe Leiterband
        if (typeof testcase.farbe_leiterband != "undefined") {
            await this.page.locator('.leiterband > ul > li').locator(":scope:not(.ng-hide)").locator('img[alt=' + testcase.farbe_leiterband + ']').click();
        }
