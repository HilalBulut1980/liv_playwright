...
exports.Raffrollo = class Raffrollo {
    ...
    async startFromConfigurator(testcase) {
        ...
        await this.page.goto('/raffrollo/raffrollo-konfigurator')
    ...
    async configureRaffrollo(testcase) {
        ...
        //zusätze
        let kette_mwst = (jsonLogic.apply({ '*': [jsonLogic.apply({ '/': [testcase.ketten_preis, 119] }), testcase.vat] })).toFixed(2);
        ...
        let streichPreis = (jsonLogic.apply({ '+': [testcase.grundpreis, testcase.ketten_preis, testcase.befestigung_preis] })).toFixed(2)
        let streichPreisTotal = (jsonLogic.apply({ '*': [streichPreis, testcase.anzahl] })).toFixed(2)
        let redPreis = (jsonLogic.apply({ '+': [grundPreis_red, testcase.ketten_preis, testcase.befestigung_preis] })).toFixed(2)
        ...
        // set Kettenart
        await this.page.locator('span').getByText(testcase.art_kette).first().click();

        // set Kettenfarbe
        if (testcase.art_kette == "PVC") {
            // await this.page.locator('span').getByText(testcase.farbe_kette).first().click();
            await this.page.locator('#bedienung-normal li').getByText(testcase.farbe_kette).click();
        }

        // set Bedienseite
        await this.page.locator('span').getByText(testcase.bedienseite).first().click();
