...
test('DF20 genormt - check comfort / non-comfort with Velux VX Flügeltyp 21 A | Holz', async ({ page }) => {
    ...
    await page.selectOption(('.dfselect:nth-of-type(3) select'), '21 A | Holz')
    ...
    await expect(page.locator("#configurator-navigation div.old-price > span")).toContainText(/187,00/);
    await expect(page.locator("#configurator-navigation div.special-price > span")).toContainText(/84,15/)
    await expect(page.locator("#configurator-price-cart div.old-price > span")).toContainText(/187,00/)
    await expect(page.locator("#configurator-price-cart div.special-price > span")).toContainText(/84,15/)
    ...
    // Streichpreis einzel 
    await expect(page.locator('span[class="cart-price"] > div > span[style="text-decoration: line-through;"]')).toContainText('187,00');
    // red. Einzelpreis
    await expect(page.locator('span[class="cart-price"] > div > .price')).toContainText('84,15');
    //Streichpreis gesamt
    await expect(page.locator('span[class="cart-price cart-zwischensumme"] > div > span[style="text-decoration: line-through;"]')).toContainText('187,00');
    //red. Gesamtpreis
    await expect(page.locator('span[class="cart-price cart-zwischensumme"] > div > .price')).toContainText('84,15');
