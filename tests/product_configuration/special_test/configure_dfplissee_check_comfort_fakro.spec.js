...
test('DF20 genormt - check comfort / non-comfort with Fakro FYP-V 94 / 180 | Holz', async ({ page }) => {
    ...
    await page.selectOption(('.dfselect select'), 'Fakro')
    await page.selectOption(('.dfselect:nth-of-type(2) select'), 'FYP-V')
    await page.selectOption(('.dfselect:nth-of-type(3) select'), '94 / 180 | Holz')
    // --> 1633 x 781 comfort PG4
    ...
    await expect(page.locator("#configurator-navigation div.old-price > span")).toContainText(/619,00/);
    await expect(page.locator("#configurator-navigation div.special-price > span")).toContainText(/278,55/)
    await expect(page.locator("#configurator-price-cart div.old-price > span")).toContainText(/619,00/)
    await expect(page.locator("#configurator-price-cart div.special-price > span")).toContainText(/278,55/)
    ...
    // Streichpreis einzel 
    await expect(page.locator('span[class="cart-price"] > div > span[style="text-decoration: line-through;"]')).toContainText('619,00');
    // red. Einzelpreis
    await expect(page.locator('span[class="cart-price"] > div > .price')).toContainText('278,55');
    //Streichpreis gesamt
    await expect(page.locator('span[class="cart-price cart-zwischensumme"] > div > span[style="text-decoration: line-through;"]')).toContainText('619,00');
    //red. Gesamtpreis
    await expect(page.locator('span[class="cart-price cart-zwischensumme"] > div > .price')).toContainText('278,55');
